...
const TyreManagement = () => {
  ...
  const checkTyreStatus = async () => {
    try {
      ...
      const response = await axiosInstance.get(
        `/check/${vehicleId}/${tyrePosition}?currentKm=${currentKm}`
      );
  ...
  const handleClickAction = (plateNumber, tyrePosition) => {
    console.log(
      `Clicked action for vehicle with plate number: ${plateNumber} ${tyrePosition}`
    );
    setStatusQuery({
  ...
  const generatePDF = (data) => {
    ...
    doc.setFontSize(18);
    doc.text("Tyre Records", doc.internal.pageSize.getWidth() / 2, 20, {
      align: "center",
    });
    doc.setFontSize(12);
    ...
    <div className="bg-white p-6  rounded-lg w-full md:w-3/4 lg:w-2/3 xl:w-1/2 relative">
        ...
        ✕</button>
            ...
            >
              <input
                ...
                onChange={(e) =>
                  setStatusQuery({ ...statusQuery, currentKm: e.target.value })
                }
      ...
      <div className="bg-white p-2  rounded-lg">
              ...
              {currentTyres.map((tyre) => (
                <tr key={tyre.tyreSerielNo} className="hover:bg-gray-100">
                  ...
                  <td className="border p-3">
                    <button
                      onClick={() =>
                        handleClickAction(tyre.vehicle._id, tyre.tyrePosition)
                      }
        ...
        <div className="mt-4 flex justify-center space-x-4">
          {Array.from(
            { length: Math.ceil(filteredTyres.length / itemsPerPage) },
            (_, index) => (
              <button
                key={index + 1}
